// PicoTTL - Example 04: several horizontal lines
//
// Validates repeated Graphics::drawHorizontalLine() calls and vertical
// linearity: white lines every 25 rows (including the first and last
// visible row), full width.
//
...
int main() {
    ...
    gfx.clear();
    for (std::int32_t y = 0; y < display.height(); y += 25) {
        gfx.drawHorizontalLine(0, y, display.width(), picottl::colors::kWhite);
    }
    gfx.drawHorizontalLine(0, display.height() - 1, display.width(),
                           picottl::colors::kWhite);
