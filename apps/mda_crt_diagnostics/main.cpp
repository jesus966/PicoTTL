// PicoTTL MDA CRT Diagnostics
//
// OFFICIAL PicoTTL APPLICATION - the permanent hardware and framework
// validation suite (see DiagnosticPattern.hpp for the policy). It
// exercises exactly the same public API that normal applications use -
// including dynamic video mode switching - and runs unchanged on every
// backend: only the DisplayDevice construction below is
// backend-specific. It never relies on concrete mode geometry: all
// sizes are queried through the Display API, so adjusting e.g. the
// overscan constraints after hardware validation requires zero changes
// here.
//
...
constexpr VideoMode kModes[] = {VideoMode::IbmMda, VideoMode::MdaOverscan};
constexpr std::size_t kModeCount = sizeof kModes / sizeof kModes[0];

constexpr std::size_t kStorageBytes = PackedFramebuffer2::bytesFor(
    modeWidth(VideoMode::MdaOverscan), modeHeight(VideoMode::MdaOverscan));
static_assert(kStorageBytes >= PackedFramebuffer2::bytesFor(
...
PackedFramebuffer2 gFramebuffers[kModeCount] = {
...
// ---------------------------------------------------------------------------
// Mode switching through the PUBLIC API only: this application is the
// first consumer of dynamic mode changes and validates the framework
// path every backend will use.
// ---------------------------------------------------------------------------
...
/// Mode selection pattern: each press applies the next supported mode
/// (immediate feedback on the CRT); after "CONTINUE" the press advances
/// to the next pattern like everywhere else.
...
AdjustableRasterPattern gAdjustableRaster;
...
BarsPattern gVerticalBars{"Vertical bars", true, 16};
BarsPattern gHorizontalBars{"Horizontal bars", false, 16};
IntensityPattern gIntensity;
StaircasePattern gStaircase;
...
DiagnosticPattern* const gPatterns[] = {
    ...
    &gFineVStripes,  &gFineHStripes,  &gVerticalBars,    &gHorizontalBars,
    &gIntensity,     &gStaircase,     &gTextQuality,     &gAnimation,
};
...
int main() {
    ...
    rp2350::MdaDisplayDevice::Config config;
    config.hsyncPin = 2;      // VSYNC on GPIO 3; GPIO 4 internal, unwired.
    config.pixelPins[0] = 19; // VIDEO
    config.pixelPins[1] = 20; // INTENSITY
    config.framebuffer = framebufferFor(VideoMode::IbmMda);

    rp2350::MdaDisplayDevice device(config);
    Display display(device);
    Text text(display.graphics(), fonts::kIbmMda9x14);

    const bool ok = display.begin(VideoMode::IbmMda);
